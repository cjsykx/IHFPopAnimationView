//
//  UIViewController+IHFPopup.m
//  IHFPopAnimationView
//
//  Copyright © 2016年 chenjiasong. All rights reserved.
//

#import "UIViewController+IHFPopup.h"
...
NSString const *PopupKey = @"IHPopupkey";
NSString const *BlurViewKey = @"IHFadeViewKey";
NSString const *UseBlurForPopup = @"IHUseBlurForPopup";

@implementation UIViewController (IHFPopup)
...
- (void)setUseBlurForPopup:(BOOL)useBlurForPopup {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0 && useBlurForPopup) {
        ...
        objc_setAssociatedObject(self, &UseBlurForPopup, [NSNumber numberWithBool:NO], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        objc_setAssociatedObject(self, &UseBlurForPopup, [NSNumber numberWithBool:useBlurForPopup], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
